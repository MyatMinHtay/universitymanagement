...
<script>
    $(document).ready(function () {
        ...
        if (!originalTableHtml) {
            originalTableHtml = $('#faculty-list').html();
        }
        ...
        $('.filter-pill').on('click', function() {
            ...
            const searchQuery = $('#searchfaculty').val();
        ...
        function displaySearchResults(data, query) {
            ...
            if (data.length === 0) {
                ...
                $('#search-message').show().html(`<p class="text-info"><i class="bi bi-search"></i> Found ${data.length} faculty member(s) matching "${query}" in ${filterText}</p>`);
            }
        ...
        function resetToOriginal() {
            $('#faculty-list').html(originalTableHtml);
            $('#search-message').hide();
